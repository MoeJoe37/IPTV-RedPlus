...
fun PlayerScreen(session: Session, container: AppContainer, item: ContentItem?, onBack: () -> Unit) {
    ...
    val streamUrl = remember(session.accountKey, item.id, item.type) { container.contentRepository.buildStreamUrl(session, item) }
    ...
    DisposableEffect(player, resizeMode) {
        val listener = object : Player.Listener {
            override fun onPlayerError(playbackException: PlaybackException) {
                error = playbackException.message ?: "Playback error"
            }
        }
        player.addListener(listener)
        onDispose {
            val lastPosition = player.currentPosition.coerceAtLeast(0L)
            val lastDuration = player.duration.coerceAtLeast(0L)
    ...
    LaunchedEffect(item.id) {
        ...
        if (history != null && item.type != ContentType.LIVE && item.type != ContentType.EVENT && history.positionMs > 0) player.seekTo(history.positionMs)
    ...
    Box(
                ...
                detectTapGestures(
                    ...
                    onDoubleTap = { offset ->
                        if (!locked && player.isCurrentMediaItemSeekable) {
                            if (offset.x < size.width / 2) player.seekTo((player.currentPosition - 10_000).coerceAtLeast(0)) else player.seekTo(player.currentPosition + 10_000)
                        }
                    }
                )
            }
        ...
        AndroidView(
            factory = { ctx -> PlayerView(ctx).apply { useController = true; this.player = player; this.resizeMode = resizeMode.media3 } },
            update = { view -> view.player = player; view.resizeMode = resizeMode.media3; view.useController = !locked },
        ...
        if (overlay) {
            PlayerOverlay(
                ...
                error = error,
                onBack = onBack,
                onRetry = { error = null; player.prepare(); player.playWhenReady = true },
...
private fun PlayerOverlay(
    ...
    Column(Modifier.fillMaxSize().padding(18.dp), verticalArrangement = Arrangement.SpaceBetween) {
        GlassPanel(Modifier.fillMaxWidth()) {
            Row(Modifier.padding(12.dp), verticalAlignment = Alignment.CenterVertically) {
                ...
                Column(Modifier.weight(1f)) {
                    ...
                    Text(if (item.type == ContentType.LIVE || item.type == ContentType.EVENT) "Live playback" else "${msToTime(position)} / ${msToTime(duration)}", color = PremiumMuted)
                }
        ...
        if (error != null) {
            GlassPanel(Modifier.fillMaxWidth()) { Row(Modifier.padding(16.dp), verticalAlignment = Alignment.CenterVertically) { Text(error, color = MaterialTheme.colorScheme.error, modifier = Modifier.weight(1f)); Button(onClick = onRetry) { Text("Retry") } } }
        }
        if (duration > 0 && item.type != ContentType.LIVE && item.type != ContentType.EVENT) {
            GlassPanel(Modifier.fillMaxWidth()) { Column(Modifier.padding(12.dp)) { Slider(value = position.toFloat().coerceIn(0f, duration.toFloat()), onValueChange = { onSeek(it.toLong()) }, valueRange = 0f..duration.toFloat()); Text("Double tap left/right to seek. Swipe left side for brightness, right side for volume.", color = PremiumMuted) } }
        } else {
            GlassPanel(Modifier.fillMaxWidth()) { Text("Tap for controls. Double tap seeks where supported. Swipe for brightness/volume. Media3 controller exposes audio/subtitle track options when streams provide them.", Modifier.padding(14.dp), color = PremiumMuted) }
        }
    }
}
