...
class ContentRepository(private val client: XtreamClient, private val fileCache: FileCache, private val gson: Gson) {
    ...
    fun buildStreamUrl(session: Session, item: ContentItem): String = buildStreamUrls(session, item).first()

    fun buildStreamUrls(session: Session, item: ContentItem): List<String> = when (item.type) {
        ContentType.LIVE, ContentType.EVENT -> client.buildLiveUrls(session.serverUrl, session.username, session.password, item.id, item.directSource)
        ContentType.MOVIE -> client.buildMovieUrls(session.serverUrl, session.username, session.password, item.id, item.streamExtension)
        ContentType.SERIES, ContentType.EPISODE -> client.buildSeriesUrls(session.serverUrl, session.username, session.password, item.id, item.streamExtension)
    }.distinct()
